...
func (s *ConfigService) Handle(ctx context.Context, route string, body []byte) (shared.Object, error) {
	...
	if route == "/api/config" && len(body) == 0 {
		...
		return shared.Object{"config": configstore.Editable(c), "path": s.Store.Path}, nil
	}
	var input struct {
	...
	result := shared.Object{"config": configstore.Editable(c), "path": s.Store.Path}
	if copiedName != "" {
...
func StartSetup(service *ConfigService, assets fs.FS) (*SetupServer, error) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "GET" {
			...
			if shared.Contains([]string{"index.html", "config-ui.js", "config.css", "desktop-ui.js", "desktop.css", "bridge.js", "shell-ui.js", "select-ui.js"}, name) {
