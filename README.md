...
发布仓库现已公开，无需登录即可下载安装包。安装包尚未配置代码签名证书，Windows 可能显示“发布者未知”。

**从 0.2.0 / 0.2.1 升级**：这两个旧版本内置了私有发布源开关，需要先手动下载并覆盖安装 0.2.2 或更新版本；之后可直接在 App 中检查、下载和重启更新。覆盖安装会保留本机模型配置。
...
- **桌面版（0.2.2 起）**：在 App 中点击“检查更新”，发现新版本后点击下载，再点击“重启并更新”。然后点击“更新插件”，并在 Codex 新建对话。0.2.0 / 0.2.1 需要先按快速开始中的说明手动升级一次。
...
2. 提交并推送源码，再推送与版本一致的标签，例如 `v0.2.3`。
...
发布目标在 `desktop/release.json` 中配置。当前公开仓库使用 `private: false`，App 可免登录读取 Release 并下载更新。模型连接、API Key、任务记录保存在使用者本机，不属于源码或安装包。打包使用明确的文件清单，发布前会检查实际产物，拒绝带入本机配置和凭据文件。
