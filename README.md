...
**一个 worker（连接）可以配置多个模型**，共用地址和 Key。一次勾选并保存后，在 Codex 中直接切换「连接名 · 模型名称」，无需回 App 更换默认模型。每个模型使用自己的兼容策略与上下文容量；「跟随 App 选择」才会随 App 默认值变化。
...
- 模型可独立选择 Gemini、DeepSeek、Kimi、豆包、MiniMax、GLM 等兼容策略，保留 CPA 通用模式；补齐结构化思考和工具历史回传，详见[兼容说明](docs/model-compatibility.md)。
- 一个 worker 下的多个模型可直接在 Codex 中切换；具体模型使用固定标识，调整 App 默认值不会误切已经选定的模型。
- 已选模型可独立设置上下文长度，压缩阈值随容量调整；保存后重启 Codex 生效，并保留关闭挟持模式时的原配置恢复。
- 优化外部模型的进度提示要求，并明确识别缺少完成事件的断流，避免将不完整响应当作成功。
- 模型列表增加移除按钮和确认弹窗，手动模型可彻底移除；移除默认模型时自动选用剩余已选模型。
- 连接信息、任务分工和高级设置直接显示配置字段，移除重复标题与说明；弹窗和浮动提示不再以中文句号结尾。
