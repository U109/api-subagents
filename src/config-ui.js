...
/** 带会话令牌访问本地配置 API，并把服务端错误转换为可显示的异常。 */
  ...
  byId('models').innerHTML = entries.length
    ...
    <div class="model ${name === selected ? 'active' : ''}">
      <button class="model-select" data-name="${esc(name)}" ${name === selected ? 'aria-current="true"' : ''}>
        <span class="model-dot" aria-hidden="true"></span><span class="model-copy"><strong>${esc(name)}</strong><small>${esc(p.model || '尚未选择模型')}</small></span>
      </button>
