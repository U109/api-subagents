...
try {
    ...
    $fixture = '{"version":1,"models":{"demo":{"protocol":"compatible","model":"mock-model","baseUrl":"http://127.0.0.1:9/v1","reasoningEffort":"low"}}}'
    ...
    if (-not $result.ok -or -not $result.configLoaded -or $result.tabs -ne 4 -or $result.reasoningOptions -ne 7 -or $result.reasoningValue -ne 'low' -or $result.expanders -ne 0) { throw ('Desktop smoke failed: ' + ($result | ConvertTo-Json -Compress)) }
    Write-Host ('Verified WebView2, Go bindings, four tabs and reasoning selector: ' + $result.version)
} finally {
